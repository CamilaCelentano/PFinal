...
@Named(value = "modificarUsuario") // JEE8
@SessionScoped // JEE8
public class ControladorModificarUsuario implements Serializable {
	...
	public void modificarCom() {
		try {
			comunBean.actualizar(com);
			FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Usuario comun modificado correctamente.", "");
			FacesContext.getCurrentInstance().addMessage(null, facesMsg);
		} catch (ServiciosException e) {
			FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_ERROR, e.getMessage(), "");
			FacesContext.getCurrentInstance().addMessage(null, facesMsg);
	...
	public void modificarExp() {
		try {
			expertoBean.actualizar(exp);
			FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Experto modificado correctamente.",
					"");
			FacesContext.getCurrentInstance().addMessage(null, facesMsg);
		} catch (ServiciosException e) {
			FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_ERROR, e.getMessage(), "");
